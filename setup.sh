...
setup_h4l() {
    ...
    if [ "${H4L_SETUP}" = "1" ]; then
        >&2 echo "the hto4l analysis was already succesfully setup"
    ...
    if [ "${CF_REMOTE_JOB}" != "1" ]; then
        cf_setup_interactive_body() {
            ...
            query CF_DATA "Local data directory" "\$H4L_BASE/data" "./data"
            query CF_STORE_NAME "Relative path used in store paths (see next queries)" "h4l_store"
            query CF_STORE_LOCAL "Default local output store" "\$CF_DATA/\$CF_STORE_NAME"
            query CF_WLCG_CACHE_ROOT "Local directory for caching remote files" "\$CF_DATA/h4l_cache"
            ...
            query H4L_BUNDLE_CMSSW "Install and bundle CMSSW sandboxes for job submission?" "True"
        }
    ...
    export LAW_HOME="${H4L_BASE}/.law"
    export LAW_CONFIG_FILE="${H4L_BASE}/law.cfg"
...
main() {
    ...
    if setup_h4l "$@"; then
        cf_color green "hto4l analysis successfully setup"
        return "0"
    else
        local code="$?"
